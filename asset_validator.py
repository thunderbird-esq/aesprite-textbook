...
class AssetValidator:
    ...
    def __init__(self, config_path: str = "config/master_config.yaml") -> None:
        ...
        if (
            self.config
            and "aesthetic_rules" in self.config
            and "color_distribution" in self.config["aesthetic_rules"]
        ):
            ...
            logger.info("Color limits from defaults: orange=30%, acid=10%")
        ...
        if (
            ...
            logger.info("Rotation limits from defaults: text=5°, containers=15°, photos=10°")
    ...
    def validate_image_asset(self, image_path: str) -> List[str]:
        ...
        try:
            ...
            if width == 0 or height == 0:
                violations.append(
                    f"Invalid dimensions: {width}x{height} - " f"Image has zero width or height"
                )
                logger.error("Image has zero dimensions")

            if width > 10000 or height > 10000:
                violations.append(
                    f"Excessive dimensions: {width}x{height} - " f"Maximum dimension is 10000px"
                )
    ...
    def _check_color_distribution(self, image: Image.Image) -> List[str]:
        ...
        try:
            ...
            orange_mask = np.all(np.abs(pixels - nickelodeon_color) < tolerance, axis=2)
            ...
            green_mask = np.all(np.abs(pixels - goosebumps_color) < tolerance, axis=2)
        ...
        try:
            ...
            for page_key in ["left_page", "right_page"]:
                ...
                for element in elements:
                    ...
                    spine_violations = self._check_spine_intrusion(element_id, position, dimensions)
    ...
    def _check_spine_intrusion(
        self, element_id: str, position: List[int], dimensions: List[int]
    ) -> List[str]:
    ...
    def _check_rotation_limits(
        self, element_id: str, element_type: str, rotation: float
    ) -> List[str]:
    ...
    def _check_safe_zones(
        self, element_id: str, page_key: str, position: List[int], dimensions: List[int]
    ) -> List[str]:
...
Examples:
  ...
  python asset_validator.py --layout spread.yaml --config my_config.yaml
    ...
    group.add_argument("--prompt", type=str, help="Validate a prompt file for anachronistic terms")
    group.add_argument(
        "--image", type=str, help="Validate an image asset (dimensions, format, colors)"
    )
    group.add_argument("--layout", type=str, help="Validate a YAML layout configuration")
