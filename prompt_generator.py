...
class PromptGenerator:
    ...
    def _generate_prompts_from_template(self, template_key: str, config: Dict) -> tuple:
        ...
        variables = {
            ...
            'DIMENSIONS': f"{config.get('dimensions', [800, 600])[0]}x{config.get('dimensions', [800, 600])[1]}",
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Generate XML prompts from element configurations'
    )
    parser.add_argument(
        '--config',
        required=True,
        help='Path to element configuration YAML file'
    )
    parser.add_argument(
        '--output',
        required=True,
        help='Output path for XML prompt file'
    )
    parser.add_argument(
        '--templates-dir',
    ...
    parser.add_argument(
        '--validate-only',
        action='store_true',
        help='Only validate config, do not generate prompt'
    )
